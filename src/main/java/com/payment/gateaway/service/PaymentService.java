...
import com.payment.gateaway.model.*;
...
public class PaymentService {
    ...
    public Map<String, Object> createPaymentLink(PaymentRequest paymentRequest, String jwtToken) {
        String promoCode = paymentRequest.getPromo_code();

        List<Cart> cartItems = getCartItemsFromSessionOrDatabase(jwtToken);
        ...
        for (Cart cartItem : cartItems) {
            Product product = productRepository.findById(cartItem.getProduct_id()).orElse(null);

            if (product != null) {
                Map<String, Object> itemDetails = createPaymentLinkRequest(product, cartItem.getQuantity(), promoCode);
        ...
        Map<String, Object> transactionDetails = createTransactionDetails(cartItems, itemDetailsList, promoCode);
    ...
    private Map<String, Object> createPaymentLinkRequest(Product product, int quantity, String promoCode) {
    ...
    private Map<String, Object> createTransactionDetails(List<Cart> cartItems, List<Map<String, Object>> itemDetailsList, String promoCode) {
        ...
        for (Cart cartItem : cartItems) {
            Product product = productRepository.findById(cartItem.getProduct_id()).orElseThrow(() -> new NotFoundException("Id Not Found"));

            if (product != null) {
                if (promoCode == null || promoCode.isEmpty()) {
                    double itemGrossAmount = product.getPrice() * cartItem.getQuantity();
                    totalGrossAmount += itemGrossAmount;
                } else {
                    double discountedPrice = product.getPrice() * (1 - getDiscountPercentageFromMidtrans(promoCode));
                    double itemGrossAmount = discountedPrice * cartItem.getQuantity();
                    totalGrossAmount += itemGrossAmount;
                }
            }
        }
    ...
    private List<Cart> getCartItemsFromSessionOrDatabase(String jwtToken) {
        Claims claims = jwtUtils.decodeJwt(jwtToken);
        String email = claims.getSubject();
        User user = userRepository.findByEmail(email).orElseThrow(() -> new NotFoundException("Email Not Found"));
